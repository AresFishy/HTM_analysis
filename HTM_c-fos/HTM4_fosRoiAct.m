...
for animal = animals
    ...
    for day = days(1:2:14)
        ...
        for tp = 1:6
            fosAct{animal,daycnt}(:,tp) = (tmp2arcAct{animal,daycnt}(:,tp))./low8Arc(tp);
        end
        %Calculate mean Arc expression
        if ~isempty(fosAct{animal,daycnt})
            mArcAct{animal,daycnt} = nanmean(fosAct{animal,daycnt},2);
        else
            mArcAct{animal,daycnt} = NaN(size(fosAct{animal,1},1),1);
        end
        %Calculate the difference in Arc expression across the 6 recordings
        arcDiff{animal,daycnt} = diff(fosAct{animal,daycnt},1,2);
        ...
        if ~isempty(arcDiff{animal,daycnt})
            ...
            mArcDiff{animal,daycnt} = NaN(size(fosAct{animal,1},1),1);
        end
    end
end
...
for siteID=animals
    for tp=1:7
        a_ind{siteID,tp}=zeros(size(fosAct{siteID,tp},1),1);
        [B,I]=sort(nanmean(fosAct{siteID,tp}(:,:),2),'descend');
        a_ind{siteID,tp}(sort(I(1:round(size(fosAct{siteID,tp},1)/perc_val))),1)=1;
    end
end
...
for siteID=animals
    for tp=1:7
        l_ind{siteID,tp}=zeros(size(fosAct{siteID,tp},1),1);
        [B,I]=sort(nanmean(fosAct{siteID,tp}(:,:),2),'ascend');
        l_ind{siteID,tp}(sort(I(1:round(size(fosAct{siteID,tp},1)/perc_val))),1)=1;
    end
end
