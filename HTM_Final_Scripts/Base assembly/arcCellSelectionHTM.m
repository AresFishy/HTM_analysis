...
for day = 1:7
    ...
    for animal = 1:4
        if ~isempty(act{animal,day})
            ...
            downArc{animal,day} = tmp(tmp_sort(1:round(length(tmp))/5)); %20% largest decrease Arc expression cells
            upArc{animal,day} = tmp(tmp_sort(end-round(length(tmp))/5:end)); %20% largest increase Arc expression cells
