...
%%
%plot mean Arc expresion
figure;hold on
for siteID=1:2
    for tp=1:7
        ArcExp(siteID,tp,1) = nanmean(nanmean(arcAct{siteID,tp},2));
        ArcExp(siteID,tp,2) = nanSEM(nanmean(arcAct{siteID,tp},2));
    end
end

shadedErrorBar(1:7,(ArcExp(4,:,1)),(ArcExp(4,:,2)))
xlabel('tps');
...
for ind=1:3
    ...
    plotSEM(squeeze(nanmean(ArcCorr(sel_c,:,:),2))')
end
xlabel('tps');
ylabel('change in Arc expression [%]');
...
for ind=1:2
    ...
    line([0 30],[0 0],'color','k')
    line([10 10],[0 0.03],'color','k')
    title([num2str(ArcLevel{ind}) ' c-fos' ])
end
