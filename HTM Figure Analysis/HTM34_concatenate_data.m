function [act, airPuff, lickL, lickR, grat, tone, rewL, rewR, blink, pupilD, puffON, aPuffON, pPuffON, rewLON, rewRON, rewON, pRewON, aRewLON, pRewLON, aRewRON,...
...
for day = 1:7
    for animal = animals
        ...
        tmpaPuffON{animal,day} = find(tmpptAct > 1 & tmpptAct < 4)+1; %Puffs where the mouse licked (active)
        if length(tmpaPuffON{animal,day}) > 1
            aPuffON{animal,day} = tmpaPuffON{animal,day}(find(diff(tmpaPuffON{animal,day})~=1));
        else
            aPuffON{animal,day} = tmpaPuffON{animal,day};
        end
        ...
        if ~isempty(rewLON{animal,day});
            for trial = 1:length(rewLON{animal,day})
                ...
                if max(abs(lickL{animal,day}(tON-5:tON+1))) > 0.15
                    rew1id(trial) = 1;
                else
                    rew1id(trial) = 0;
                end
        ...
        if ~isempty(rewRON{animal,day});
            for trial = 1:length(rewRON{animal,day})
                ...
                if max(abs(lickR{animal,day}(tON-5:tON+1))) > 0.15
                    rew2id(trial) = 1;
                else
                    rew2id(trial) = 0;
                end
